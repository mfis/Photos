 ...
 * Copyright (C) 2009 BalusC <br>
 * Original code published under GNU Lesser General Public License
 * 
...
public class FileDownloadUtil {
	...
	public void process(HttpServletRequest request, HttpServletResponse response, File file, boolean content)
		...
		boolean isEncrypted = StringUtils.endsWith(fileName, AES.FILE_SUFFIX);
		long length;
		if (isEncrypted) {
			fileName = StringUtils.removeEnd(fileName, AES.FILE_SUFFIX);
			InputStream in = new FileInputStream(file);
			length = AES.readDecryptedSize(in);
			try {
				in.close();
			} catch (IOException e) {
				// ignore
